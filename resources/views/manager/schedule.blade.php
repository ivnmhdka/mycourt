<x-app-layout>
                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="GET" action="{{ route('manager.schedule') }}" class="flex flex-col md:flex-row gap-4 mb-8 items-end">
                        <div class="w-full md:w-1/3">
                            ...
                            <select name="field_id" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">
                        ...
                        <div class="w-full md:w-1/3">
                            ...
                            <input type="date" name="date" value="{{ $date }}" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">
                        </div>
        ...
        function scheduleManager(initialBlockedSlots, initialBookedSlots) {
            return {
    ...
    </script>                blockedSlots: initialBlockedSlots || [],
                bookedSlots: initialBookedSlots || {}, 
                cancelledBookings: [],
                selectedBooking: null, 
                ...
                cancelSelectedBooking() {
                    ...
                    if (this.manualBookings[time]) {
                        return 'bg-blue-50 border-blue-200 hover:bg-blue-100 shadow-sm';
                    }

                    // 3. Check if Booked (Red)
                    if (isBookedOriginal) {
                        return 'bg-red-50 border-red-200 hover:bg-red-100'; 
                    }

                    // 4. Check if Blocked (Gray)
                    if (this.blockedSlots.includes(time)) {
                        return 'bg-gray-200 border-gray-400 hover:bg-gray-300';
                    }

                    // 5. Default: Open (Emerald)
                    return 'bg-emerald-50 border-emerald-200 hover:bg-emerald-100';
                },
                ...
                toggleBlock(time, isBooked) {
                    // Case A: Editing existing Booking
                    if (isBooked) {
                        if (this.cancelledBookings.includes(time)) {
                            this.cancelledBookings = this.cancelledBookings.filter(t => t !== time);
                            return;
                    ...
                    // Case B: Remove Manual Booking if exists
                    if (this.manualBookings[time]) {
                        delete this.manualBookings[time];
                        this.manualBookings = { ...this.manualBookings };
                        return;
                    }

                    // Case C: Remove Block if exists
                    if (this.blockedSlots.includes(time)) {
                        this.blockedSlots = this.blockedSlots.filter(t => t !== time);
                        return;
                    }

                    // Case D: Slot is Empty -> Prompt Action (Block vs Manual)
                    this.actionSlotTime = time;
                    window.dispatchEvent(new CustomEvent('open-modal', { detail: 'slot-action-modal' }));
                },

                confirmBlock() {
                    if (this.actionSlotTime) {
                        this.blockedSlots.push(this.actionSlotTime);
                        this.actionSlotTime = null;
                        window.dispatchEvent(new CustomEvent('close-modal', { detail: 'slot-action-modal' }));
                    }
                },
                ...
                openManualBookingForm() {
                    ...
                    setTimeout(() => {
                        window.dispatchEvent(new CustomEvent('open-modal', { detail: 'manual-booking-modal' }));
                    }, 200);
                },

                saveManualBooking() {
                   if (this.actionSlotTime && this.manualGuestName.trim() !== '') {
                       this.manualBookings[this.actionSlotTime] = this.manualGuestName;
                       this.manualBookings = { ...this.manualBookings };
                   }
                   this.actionSlotTime = null;
                   window.dispatchEvent(new CustomEvent('close-modal', { detail: 'manual-booking-modal' }));
                },
                    }
